...
async def main():
  async with GateClient(GATE_API_KEY, GATE_API_SECRET) as gate_client:
    async with HyperliquidClient(HYPERLIQUID_SECRET_KEY, HYPERLIQUID_ACCOUNT_ADDRESS) as hyperliquid_client:
      ...
      gate_book = await gate.estimate_fill_price('ENA', 500000, PositionSide.SHORT, depth=50)
      hyperliquid_book = await hyperliquid.estimate_fill_price('ENA', 500000, PositionSide.SHORT, depth=50)
